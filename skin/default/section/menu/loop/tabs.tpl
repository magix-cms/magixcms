{if $menuData}
    ...
    <li class="menu-item{if $item.subdata} mgdropdown{/if}{if $item.active && !(isset($smarty.get.idclc) && $smarty.get.idclc == {#id_mainCat#} && $key == 'catalog')} active{/if}">
        {if $item.subdata}
        ...
        {if $item.subdata}
            {if isset($menuclass) && $menuclass === 'mega-dropdown'}
            <div class="dropdown hidden-xs"{if $microData} itemprop="hasPart" itemscope itemtype="http://schema.org/SiteNavigationElement"{/if}>
                {foreach $item.subdata as $child}
                    <div class="col-sm-3"{if $microData} itemprop="name"{/if}>
